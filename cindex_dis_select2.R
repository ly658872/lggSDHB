cindex_dis_select2 <- function(object, # output of ML.Dev.Prog.Sig mode = "all",'single' or 'double'
  ...
  if (model != "SOD") {
    ...
    p1 <- ggplot(data = cindex, aes(x = ID, y = Cindex, fill = ID)) +
      ...
      geom_text(aes(label = Cindex), position = position_dodge(width = 0.9),
                vjust = 0.5, hjust = 1.2, size = 10) +
      theme(
